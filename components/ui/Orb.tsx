...
export default function Orb({
  ...
  `;
	...
	useEffect(() => {
		...
		const processInteraction = (x: number, y: number) => {
			if (!container) return;
			...
			const uvX = ((x - rect.left - centerX) / size) * 2.0;
			const uvY = ((y - rect.top - centerY) / size) * 2.0;
	...
	// Fallback content if WebGL fails to initialize
	if (hasError) {
		return (
			<div className="w-full h-full flex items-center justify-center bg-gradient-to-r from-green-200 via-teal-400 to-cyan-600 opacity-50 rounded-full">
